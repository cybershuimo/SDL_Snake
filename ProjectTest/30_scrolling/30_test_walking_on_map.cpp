...
class Dot
{
    public:
        ...
        void render( LTexture& sourceTexture );

        //Change collider width and height
        void changeCollider(int width, int height);

        //Show dot collision state, return true when not collision
        bool notCollision();
    ...
    private:
        ...
        //Dot collision state, x axis and y axis
        bool mCollisionStateX, mCollisionStateY;
};
...
Dot::Dot( int x, int y )
{
    ...
    mCollisionStateX = false;
    mCollisionStateY = false;
...
void Dot::move()
{
    ...
    if( ( mPosY - mCollider.h / 2 < 0 ) || ( mPosY + mCollider.h / 2 > LEVEL_HEIGHT ) )
    {
        ...
        mPosY -= mVelY;
        mCollider.y = mPosY;
        mCollisionStateY = true;
    }

}

// render specific part of sourceTexture
void Dot::render( LTexture& sourceTexture )
{
    //Set destination rectangle
    SDL_Rect sourceQuad = { mPosX - mCollider.w / 2, mPosY - mCollider.h / 2, mCollider.w, mCollider.h };

    //Show the dot
    sourceTexture.render( 0, 0, &sourceQuad );
}


//Change collider width and height
void Dot::changeCollider(int width, int height)
{
    mCollider.w = width;
    mCollider.h = height;
}
...
//Show dot collision state, leave space of 5 pixels
bool Dot::notCollision()
{
    return ( mPosX - mCollider.w / 2 > 5 ) && ( mPosX + mCollider.w / 2 < SCREEN_WIDTH - 5 ) && ( mPosY - mCollider.h / 2 > 5 ) && ( mPosY + mCollider.h / 2 < SCREEN_HEIGHT - 5 );
}
...
bool init()
{
    ...
    {
        //Create window
        gWindow = SDL_CreateWindow( "SDL Test_30_walking_on_map", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
        if( gWindow == NULL )
        {
...
bool loadMedia()
{
    ...
    if( !gHeroTexture.loadFromFile( "27_collision_detection/dot.bmp" ) )
    {
...
int main( int argc, char* args[] )
{
	...
	else
	{
		...
		{	
            ...
            //The camera that will be moving around on the screen (Map Anchor)
            Dot camera( LEVEL_WIDTH / 2, LEVEL_HEIGHT / 2 );
            
            //The hero that will be moving freely when camera meets the boundary (Sprite Anchor)
            Dot hero( LEVEL_WIDTH / 2, LEVEL_HEIGHT / 2 );
            hero.changeCollider( gHeroTexture.getWidth(), gHeroTexture.getHeight() );
			...
			while( !quit )
            {
                ...
                //Render objects
                camera.render( gLevelTexture );

                // gHeroTexture.render( ( SCREEN_WIDTH - gHeroTexture.getWidth() ) / 2, 
                //         ( SCREEN_HEIGHT - gHeroTexture.getHeight() ) / 2 );

                //Camera collides boundary, render hero as it moved
                if ( !camera.notCollision() )
                {
                    gHeroTexture.render( SCREEN_WIDTH / 2 + hero.getPosX() - camera.getPosX() - gHeroTexture.getWidth() / 2, 
                        SCREEN_HEIGHT / 2 + hero.getPosY() - camera.getPosY() - gHeroTexture.getHeight() / 2 );
                }
                //Camera not collides boundary, render hero at the center
                else
                {
                    // camera.render( gHeroTexture );
                    gHeroTexture.render( ( SCREEN_WIDTH - gHeroTexture.getWidth() ) / 2, 
                        ( SCREEN_HEIGHT - gHeroTexture.getHeight() ) / 2 );
                }
