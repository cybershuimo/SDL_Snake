// Advanced timer, put it to use
// 25_control_frame_rate_manually
...
bool init()
{
    ...
    {
        //Create window
        gWindow = SDL_CreateWindow( "SDL Test_24 Frame Rate", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
        if( gWindow == NULL )
        {
        ...
        {
            //Create vsynced renderer for window; cancelled vertical sync
...
int main( int argc, char* args[] )
{
	...
	else
	{
		...
		{	
            ...
            //The avg frames per second timer
            LTimer fpsTimer;
            //The cap timer
            LTimer capTimer;
			...
			while( !quit )
            {
                ...
                //it is possible for there to be a very small amount of time passed for the first frame 
                //and have it give us a really high fps
                ...
                //if timer not passed SCREEN_TICKS_PER_FRAME, wait?
                while ( capTimer.getTicks() < SCREEN_TICKS_PER_FRAME ) 
                {
                    //nothing?
                    SDL_Delay( SCREEN_TICKS_PER_FRAME - capTimer.getTicks() );
                }
                
                // if time passed SCREEN_TICKS_PER_FRAME, present the frame
                //Update screen
